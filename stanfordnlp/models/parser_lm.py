...
from stanfordnlp.models.dplm.trainer import Trainer
from stanfordnlp.models.lm.data import DataLoader as LMDataLoader
from stanfordnlp.models.depparse.data import DataLoader as DPDataLoader
...
def parse_args():
    ...
    parser.add_argument('--lm_file', type=str, nargs='+', default=None, help='input for langauge modeling')
    ...
    parser.add_argument('--lambd', type=float, default=1, help='trade-off between dependency parsing and language modeling')
    ...
    parser.add_argument('--lm_batch_size', type=int, default=5000)
    parser.add_argument('--max_grad_norm', type=float, default=1.0, help='Gradient clipping.')
    parser.add_argument('--log_step', type=int, default=20, help='Print log every k steps.')
    parser.add_argument('--save_dir', type=str, default='saved_models/dplm', help='Root dir for saving models.')
...
def train(args):
    ...
    lm_train_batch = LMDataLoader(args['lm_file'], args['lm_batch_size'], args, pretrain,
                                  vocab=None, evaluation=False, cutoff=args['vocab_cutoff'])
    vocab = lm_train_batch.vocab
    dp_train_batch = DPDataLoader(args['train_file'], args['batch_size'], args, pretrain,
                                  vocab=None, evaluation=False, cutoff=args['vocab_cutoff'])
    vocab['deprel'] = dp_train_batch.vocab['deprel']
    dp_train_batch = DPDataLoader(args['train_file'], args['batch_size'], args, pretrain,
                                  vocab=vocab, evaluation=False, cutoff=args['vocab_cutoff'])
    train_dev_batch = DPDataLoader(args['train_file'], args['batch_size'], args, pretrain, vocab=vocab, evaluation=True)
    dev_batch = DPDataLoader(args['eval_file'], args['batch_size'], args, pretrain, vocab=vocab, evaluation=True)
    ...
    while True:
        ...
        global_step += 1
        dp_loss, lm_loss, loss = trainer.update(dp_batch, lm_batch, eval=False)  # update step
        ...
        if global_step % args['log_step'] == 0:
            ...
            print(format_str.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), global_step,
                                    max_steps, log_loss[2], duration, log_loss[1], np.exp(log_loss[0]), current_lr))
            log_loss[:] = 0
        ...
        if global_step % args['eval_interval'] == 0:
            ...
            print("step {}: train_score = {:.4f}, dev_score = {:.4f}".format(global_step, train_score, dev_score))
            train_loss[:] = 0
