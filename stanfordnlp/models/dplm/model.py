...
class ParserLM(nn.Module):
    ...
    def __init__(self, args, vocab, emb_matrix=None, share_hid=False):
        ...
        # recurrent layers
        assert args['lstm_type'] in ('hlstm', 'wdlstm')
        if args['lstm_type'] == 'hlstm':
            self.lstm_forward = HighwayLSTM(input_size, self.args['hidden_dim'], self.args['num_layers'], batch_first=True, bidirectional=False,
                                            dropout=args['dropout'], rec_dropout=args['rec_dropout'], highway_func=torch.tanh)
            ...
            self.lstm_forward = WeightDropLSTM(input_size, self.args['hidden_dim'], self.args['num_layers'], batch_first=True, bidirectional=False,
                                               dropout=args['dropout'], weight_dropout=args['rec_dropout'])
    ...
    def forward(self, word, word_mask, wordchars, wordchars_mask, upos, xpos, ufeats,
        ...
        if self.training:
            ...
            loss = loss / wordchars.size(0)  # number of words
        else:
            loss = 0
    ...
    def lm_forward(self, word, word_mask, wordchars, wordchars_mask, upos, xpos, ufeats, pretrained,
        ...
        # print(' '.join([self.vocab['word'].id2unit(t) for t in prev_word[0]]))
        # print('-'*50)
